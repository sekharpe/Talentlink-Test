...
2. Design database schema (users, profiles, projects, proposals, contracts, messages, 
   reviews, skills).
3. Initialize Django project with Django REST Framework

WEEK 2:
1. Set up PostgreSQL (production) / SQLite (local).
2. Implement REST endpoints for registration/login (JWT), user profile CRUD.
3. Initial frontend React app skeleton and authentication flow (login/register pages).
...
WEEK 4 :  
1. Proposal model & endpoints: freelancers submit proposals; clients view/manage 
proposals.  
...
4. Deploy backend and frontend (Render/Heroku/AWS). Use PostgreSQL in production, 
   configure environment variables, CORS, SSL.
